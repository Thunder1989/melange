...
modelName = "proactiveLearning_auditor_LCBUCB_auditor_extra"
...
class _ProactiveLearning:
	...
	def select_example(self, unlabeled_list, targetNameFeatureInit, auditorLabelInit):
		...
		if len(np.unique(auditorLabelInit)) < 2:
			randomeSelectedSample = random.sample(unlabeled_list, 1)[0]
			print("randomeSelectedSample", randomeSelectedSample)
			return randomeSelectedSample
		...
		for unlabeledIdIndex in range(unlabeledIdNum):
			...
			coefDiff = np.dot(self.m_judgeClassifier.coef_, self.m_targetNameFeature[unlabeledId])
			# print("coef diff", coefDiff)
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			auditorLabelInit = (targetLabelInit == transferLabelInit)

			print("initExList\t", initExList, targetLabelInit, auditorLabelInit)
			...
			transferFeatureList = targetNameFeatureInit
			transferFlagList = auditorLabelInit

			if len(np.unique(auditorLabelInit)) > 1:
				self.m_judgeClassifier.fit(targetNameFeatureInit, auditorLabelInit)
			...
			while activeLabelNum < rounds:
				...
				exId = self.select_example(unlabeledExList, targetNameFeatureInit, auditorLabelInit) 
				...
				if transferLabel == exLabel:
					...
					transferFeatureList = np.vstack((transferFeatureList, self.m_targetNameFeature[exId]))
					transferFlagList = np.hstack((transferFlagList, 1.0))
				else:
					# wrongUntransferLabelNum += 1.0
					# transferFlagList.append(0.0)
					# transferFeatureList.append(self.m_targetNameFeature[exId])
					transferFeatureList = np.vstack((transferFeatureList, self.m_targetNameFeature[exId]))
					transferFlagList = np.hstack((transferFlagList, 0.0))
