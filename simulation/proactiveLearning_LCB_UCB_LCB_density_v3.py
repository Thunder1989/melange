"""
use density to weight the rank of unlabeled instances
density + weight*idscore
"""
...
modelName = "proactiveLearning_LCBUCB_LCB05_CbRate0002_density_v2"
...
class _ProactiveLearning:
	...
	def select_example(self, unlabeled_list):
		...
		labelIdLabelIndexMap = {} ###labelId: labelIndex
		...
		for unlabeledIdIndex in range(unlabeledIdNum):
			...
			idScore = 1-LCB
			# unlabeledIdScoreMap[unlabeledId] = idScore
			# if maxLabelIndex not in labelIndexUnlabeledIdScoreMap:
				# labelIndexUnlabeledIdScoreMap.setdefault(maxLabelIndex, {})

			# if unlabeledId not in labelIndexUnlabeledIdScoreMap[maxLabelIndex]:
				# labelIndexUnlabeledIdScoreMap[maxLabelIndex].setdefault(unlabeledId, 0.0)
			unlabeledIdScoreMap[unlabeledId] = idScore
			# labelIndexUnlabeledIdScoreMap[maxLabelIndex][unlabeledId] = idScore
			...
			if unlabeledId not in labelIdLabelIndexMap:
				labelIdLabelIndexMap.setdefault(unlabeledId, maxLabelIndex)

			# if maxLabelIndex not in labelInstanceIDMap:
			# 	labelInstanceIDMap.setdefault(maxLabelIndex, [])
			# labelInstanceIDMap[maxLabelIndex].append(unlabeledId) 
		...
		for unlabeledId in unlabeledIdScoreMap:
			unlabeledIdScoreMap[unlabeledId] = unlabeledIdScoreMap[unlabeledId]
			# print("lcb-ucb", unlabeledIdScoreMap[unlabeledId])
			unlabeledIdScoreMap[unlabeledId] += 2.0*labelDensityMap[labelIdLabelIndexMap[unlabeledId]]

			# unlabeledIdScoreMap[unlabeledId] += 2.0*labelDensityMap[labelIdLabelIndexMap[unlabeledId]]

			# print("density", labelDensityMap[labelIdLabelIndexMap[unlabeledId]])
		...
		for instanceIndex in range(10):
			densityTop10IDList.append(labelDensityMap[labelIdLabelIndexMap[sortedUnlabeledIdList[instanceIndex]]])
			labelIndexTop10IDList.append(labelIdLabelIndexMap[sortedUnlabeledIdList[instanceIndex]])
		print("density", densityTop10IDList)
		print("predict labelIndex", labelIndexTop10IDList)
		print("===========")
...
if __name__ == "__main__":
	...
	f = open('simulatedTransferLabel_500.txt')
	transferLabel = []
