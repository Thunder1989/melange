...
modelName = "proactiveLearning_LCBUCB_LCB05_CbRate0002_auditor"
...
class _ProactiveLearning:
	...
	def select_example(self, unlabeled_list):
		...
		print(unlabeledIdScoreMap[sortedUnlabeledIdList[0]], unlabeledIdScoreMap[sortedUnlabeledIdList[-1]], unlabeledIdScoreMap[sortedUnlabeledIdList[unlabeledIdNum/2]])

		# print("****************")
		# print(sortedUnlabeledIdList[:10])

		pseudoNum = int(unlabeledIdNum*0.02)
		print("pseudoNum", pseudoNum)
		pseudoIdList = []

		for pseudoIndex in range(pseudoNum):
			pseudoIdList.append(sortedUnlabeledIdList[-(pseudoIndex+1)])
		...
		return sortedUnlabeledIdList[0], pseudoIdList
	...
	def get_transfer_flag(self, transferFeatureList, transferFlagList, pseudoIdList, exId):
		predLabel = self.m_transferLabel[exId]

		pseudoFeatureList = self.m_targetNameFeature[pseudoIdList]
		pseudoFlagList = np.ones(len(pseudoIdList))
		# print(pseudoFeatureList)
		# print(pseudoFlagList)
		# print(transferFeatureList)

		if transferFeatureList:
			featureList = np.vstack((transferFeatureList, pseudoFeatureList))
			flagList = np.hstack((transferFlagList, pseudoFlagList))
		else:
			featureList = pseudoFeatureList
			flagList = pseudoFlagList

		if len(np.unique(flagList)) > 1:
			self.m_judgeClassifier.fit(featureList, flagList)
		else:
			return False, predLabel

		targetNameFeature = self.m_targetNameFeature[exId].reshape(1, -1)

		targetTransferFlag = self.m_judgeClassifier.predict(targetNameFeature)[0]

		if targetTransferFlag:
			return True, predLabel
		else:
			return False, predLabel
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			while activeLabelNum < rounds:
				...
				exId, pseudoIdList = self.select_example(unlabeledExList) 
				self.update_select_confidence_bound(exId)

				# print(idx)
				activeLabelFlag = False
				# pseudoIdList = []
				print("pseudoIdList", pseudoIdList)
				transferLabelFlag, transferLabel = self.get_transfer_flag(transferFeatureList, transferFlagList, pseudoIdList, exId)
