...
classifierParam = np.random.random_sample((1, featureDim))
...
for classifierIndex in range(1):
...
for sampleIndex in range(sampleNum):
	...
	oneProb = classifierParam.dot(sampleFeature)
	oneProb = 1.0/(np.exp(-oneProb)+1)
	sampleLabel = bernoulli.rvs(oneProb, size=1)[0]

	# print(labelRank)
	y[sampleIndex] = sampleLabel
