...
use density to filter instances, first density and then idscore
"""
...
modelName = "proactiveLearning_minMargin_LR_density"
...
class _ProactiveLearning:
	...
	def updateWeakLabeledIDList(self, transferFeatureList, transferFlagList):
		...
		for weakLabeledIndex in range(weakLabeledNum):
			weakLabeledID = self.m_weakLabeledIDList[weakLabeledIndex]

			transferFlag, predLabel = self.get_transfer_flag(transferFeatureList, transferFlagList, weakLabeledID)

			if not transferFlag:
				self.m_weakLabeledIDList.remove(weakLabeledID)

				self.update_select_confidence_bound_removeID(weakLabeledID)
