"""
proactive learning with random initialization, judge classifier to judge whether transfer learning is correct. we use max margin to select instance. And we use LCB to determine whether the transferred label is correct or not. In this script, we look into the upper bound of judge classifier, which means when the judge classifier is 100% sure about whether the transferred label is correct or not. 
"""
...
modelName = "proactive_LCB_margin_upper_FN_JC"
...
class _ProactiveLearning:
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			correctUntransferRatio = correctUntransferLabelNum
			...
			print("transferLabelNum\t", transferLabelNum, "correct ratio\t", correctTransferRatio)
		...
		print("transfer correct ratio\t", np.mean(correctTransferRatioList), np.sqrt(np.var(correctTransferRatioList)))
