...
modelName = "proactive_margin_LCB05plusMajorityVote"
...
class _ProactiveLearning:
	...
	def get_judgeClassifier_prob(self, predLabelPosRatio, judgeParam, feature, CB):
		rawProb = np.dot(judgeParam, np.transpose(feature))
		judgeProbThreshold = 0.3
		CBThreshold = 28.0
	...
	def get_transfer_flag(self, transferFeatureList, transferFlagList, exId, UnTransferLabelPosNegMap):
		...
		# 	transferFlag = True
		# else:
		transferFlag = self.get_judgeClassifier_prob(predLabelPosRatio, self.m_judgeClassifier.coef_, self.m_targetNameFeature[exId].reshape(1, -1), CB)
