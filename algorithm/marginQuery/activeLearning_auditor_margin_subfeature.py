...
modelName = "active_auditor_margin_subfeature"
...
class _ActiveLearning:
	...
	def select_example(self, unlabeled_list, targetNameFeature):
		...
		for unlabeledIdIndex in range(unlabeledIdNum):
			...
			labelPredictProb = self.m_judgeClassifier.predict_proba(targetNameFeature[unlabeledId].reshape(1, -1))[0]
	...
	def run_CV(self):
		...
		subFeatureNum = 100
		featureDim = len(self.m_targetNameFeature[0])
		selectedFeatureIndex = np.random.choice(range(featureDim), subFeatureNum)
		targetNameFeature = self.m_targetNameFeature[:,selectedFeatureIndex]
		...
		for foldIndex in range(foldNum):
			...
			targetNameFeatureTrain = targetNameFeature[train]
			# targetAuditorLabelTrain = targetAuditorLabel[train]

			targetNameFeatureTest = targetNameFeature[test]
			...
			targetNameFeatureInit = targetNameFeature[initExList]
			targetLabelInit = targetAuditorLabel[initExList]
			...
			while activeLabelNum < rounds:
				...
				exId = self.select_example(unlabeledExList, targetNameFeature) 
				# print(exId)
				...
				if transferLabel == exLabel:
					...
					targetNameFeatureIter = np.vstack((targetNameFeatureIter, targetNameFeature[exId]))
					targetLabelIter = np.hstack((targetLabelIter, [1.0]))
				else:
					# print("wrong transfer")
					targetNameFeatureIter = np.vstack((targetNameFeatureIter, targetNameFeature[exId]))
					targetLabelIter = np.hstack((targetLabelIter, [0.0]))
				...
				acc, precision, recall = self.get_pred_acc(targetNameFeatureTest, targetAuditorLabelTest, targetNameFeatureIter, targetLabelIter)
