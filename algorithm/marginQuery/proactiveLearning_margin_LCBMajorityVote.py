...
modelName = "proactive_margin_LCB05*MajorityVote"
...
class _ProactiveLearning:
	...
	def get_judgeClassifier_prob(self, UnTransferLabelPosNegMap, predLabel, judgeParam, feature, CB):
		rawProb = np.dot(judgeParam, np.transpose(feature))
		judgeProbThreshold = 0.4
		# CBThreshold = 28.0
		...
		if predLabel in UnTransferLabelPosNegMap.keys():
			predLabelNum = UnTransferLabelPosNegMap[predLabel][0]+UnTransferLabelPosNegMap[predLabel][1]
			predLabelPosRatio = UnTransferLabelPosNegMap[predLabel][0]*1.0/(predLabelNum)

		print("predLabelPosRatio\t", predLabelPosRatio)
		cbProb = sigmoid(rawProb-self.m_cbRate*CB)
		print("sigmoid(rawProb-self.m_cbRate*CB)\t", cbProb)

		if predLabelNum > 0:
			if predLabelPosRatio*cbProb > judgeProbThreshold:
				return True
			else:
				return False
		else:
	...
	def get_transfer_flag(self, transferFeatureList, transferFlagList, exId, UnTransferLabelPosNegMap):
		...
		# 	transferFlag = True
		# else:
		transferFlag = self.get_judgeClassifier_prob(UnTransferLabelPosNegMap, predLabel, self.m_judgeClassifier.coef_, self.m_targetNameFeature[exId].reshape(1, -1), CB)
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			while activeLabelNum < rounds:
				...
				if transferLabelFlag:
					...
					if exLabel == self.m_targetLabel[exId]:
						...
						print("+++++query iteration", queryIter, " transfer label\t", exLabel)
					else:
						wrongTransferLabelNum += 1.0
						print("xxxxxquery iteration", queryIter, "error transfer label\t", exLabel, "true label", self.m_targetLabel[exId])
				else:
					...
					if transferLabel == exLabel:
						print("correct untransferLabel\t", transferLabel)
						...
						print("uncorrect untransferLabel\t", transferLabel, "true label\t", exLabel)
			...
			print("correctUntransferLabelNum\t", correctUntransferLabelNum)
